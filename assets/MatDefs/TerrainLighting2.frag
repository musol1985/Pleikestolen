...
uniform float m_Altura;
uniform float m_AlturaMitad;
...
vec2 computeLighting(in vec3 wvPos, in vec3 wvNorm, in vec3 wvViewDir, in vec3 wvLightDir){
  ...
  vec4 calculateDiffuseBlend(in vec2 texCoord) {
    ...
    if(position.y<m_Altura && position.y>=m_AlturaMitad){
        vec4 diffuseColorAltura = texture2D(m_DiffuseMap_7, texCoord * m_DiffuseMap_7_scale);
       // diffuseColor = clamp(mix( diffuseColor, diffuseColorAltura, 1-(position.y-m_AlturaMitad)/(m_AlturaMitad)),0.0f,1.0f);
 diffuseColor = clamp(mix( diffuseColor, diffuseColorAltura, 1-(position.y-m_AlturaMitad)/(m_Altura-m_AlturaMitad)),0.0f,1.0f);
    }else if(position.y<m_AlturaMitad){
        vec4 diffuseColorAltura = texture2D(m_DiffuseMap_7, texCoord * m_DiffuseMap_7_scale);
        diffuseColor = mix( diffuseColor, diffuseColorAltura, 1);
    }
  ...
  vec3 calculateNormal(in vec2 texCoord) {
    ...
    vec4 calculateTriPlanarDiffuseBlend(in vec3 wNorm, in vec4 wVert, in vec2 texCoord) {
        ...
        vec4 diffuseColor = tex0 * alphaBlend.r;
          ...
          #endif
        #endif
        diffuseColor = mix( diffuseColor, tex2,255);
